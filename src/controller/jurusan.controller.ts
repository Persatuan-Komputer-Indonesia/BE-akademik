...
export const createJurusan = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!nama_jurusan) {
    throw new AppError("nama_jurusan wajib diisi", 400);
  }
...
// DELETE (soft delete kalau service kamu begitu)
...
// RESTORE (kalau pakai soft delete)
export const restoreJurusan = asyncHandler(async (req: Request, res: Response) => {
  const id = getParam(req.params.id as any);

  await JurusanService.restore(id as any);
  res.json({ message: "Jurusan berhasil di-restore" });
});